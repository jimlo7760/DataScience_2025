...
1. Data merging (weather + soybean yield)
...
print("="*80)
print("RESEARCH QUESTION 2: How does weather affect soybean yield in Illinois?")
print("="*80)
...
# Load Illinois weather data (weekly)
with open('../weather/illinois.json', 'r') as f:
    weather_data = json.load(f)
weather_df = pd.DataFrame(weather_data)
weather_df['date'] = pd.to_datetime(weather_df['date'])
weather_df['year'] = weather_df['date'].dt.year

# Load Illinois soybean yield data (annual)
with open('../crops/processed/combined_clean.json', 'r') as f:
    crops_data = json.load(f)
soybean_records = crops_data['IL_SOYBEANS']['data']
soybean_df = pd.DataFrame(soybean_records)
soybean_df['yield'] = pd.to_numeric(soybean_df['Value'], errors='coerce')
soybean_df = soybean_df[['year', 'yield']].dropna()

print(f"Weather data: {len(weather_df)} weekly records from {weather_df['year'].min()} to {weather_df['year'].max()}")
print(f"Soybean data: {len(soybean_df)} annual records from {soybean_df['year'].min()} to {soybean_df['year'].max()}")

# Aggregate weather data for GROWING SEASON (May-September for soybeans)
print("\nAggregating weather for soybean growing season (May-September)...")
weather_df['month'] = weather_df['date'].dt.month
growing_season = weather_df[weather_df['month'].between(5, 9)]
...
# Merge weather with soybean yield
merged_df = pd.merge(soybean_df, weather_annual, on='year', how='inner')
print(f"\nMerged dataset: {len(merged_df)} years of data")
print(f"Years covered: {merged_df['year'].min()} - {merged_df['year'].max()}")

# Save merged data
merged_df.to_csv('analysis_data_q2.csv', index=False)
print("Saved merged data to: analysis_data_q2.csv")
...
for i, var in enumerate(weather_vars + ['yield']):
    plt.subplot(3, 3, i+1)
    plt.hist(merged_df[var], bins=15, color='#ff7f0e', alpha=0.7, edgecolor='black')
    plt.xlabel(var)
...
plt.tight_layout()
plt.savefig('eda_distributions_q2.png', dpi=150, bbox_inches='tight')
print("Saved: eda_distributions_q2.png")
plt.close()
...
plt.title('Correlation Matrix: Weather Variables vs Soybean Yield', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig('eda_correlation_matrix_q2.png', dpi=150, bbox_inches='tight')
print("Saved: eda_correlation_matrix_q2.png")
...
for i, var in enumerate(weather_vars):
    axes[i].scatter(merged_df[var], merged_df['yield'], alpha=0.6, s=60, color='#d62728')
    axes[i].set_xlabel(var, fontsize=11)
    axes[i].set_ylabel('Soybean Yield (BU/ACRE)', fontsize=11)
...
# Time series plot
axes[5].plot(merged_df['year'], merged_df['yield'], marker='o', linewidth=2, markersize=6, color='#ff7f0e')
axes[5].set_xlabel('Year', fontsize=11)
axes[5].set_ylabel('Soybean Yield (BU/ACRE)', fontsize=11)
axes[5].set_title('Soybean Yield Over Time', fontweight='bold')
axes[5].grid(alpha=0.3)

plt.tight_layout()
plt.savefig('eda_scatter_plots_q2.png', dpi=150, bbox_inches='tight')
print("Saved: eda_scatter_plots_q2.png")
plt.close()
...
plt.tight_layout()
plt.savefig('eda_boxplots_q2.png', dpi=150, bbox_inches='tight')
print("Saved: eda_boxplots_q2.png")
plt.close()
...
print("\n" + "="*80)
print("MODEL PERFORMANCE COMPARISON - SOYBEAN YIELD")
print("="*80)
print(comparison_df.to_string(index=False))
print("="*80)

# Save comparison
comparison_df.to_csv('model_comparison_q2.csv', index=False)
...
for i, (name, y_pred, r2, rmse) in enumerate(models_to_plot):
    axes[i].scatter(y_test, y_pred, alpha=0.7, s=100, color='#ff7f0e', edgecolors='black')
...
plt.tight_layout()
plt.savefig('model_predictions_comparison_q2.png', dpi=150, bbox_inches='tight')
print("Saved: model_predictions_comparison_q2.png")
plt.close()
...
plt.tight_layout()
plt.savefig('model_performance_bars_q2.png', dpi=150, bbox_inches='tight')
print("Saved: model_performance_bars_q2.png")
plt.close()
...
ax.bar(range(len(importances)), importances[indices], color='darkorange', alpha=0.7)
ax.set_xticks(range(len(importances)))
ax.set_xticklabels([feature_cols[i] for i in indices], rotation=45, ha='right')
ax.set_ylabel('Feature Importance', fontsize=12)
ax.set_title('Random Forest: Feature Importance for Soybean Yield Prediction', fontweight='bold')
ax.grid(axis='y', alpha=0.3)

plt.tight_layout()
plt.savefig('feature_importance_rf_q2.png', dpi=150, bbox_inches='tight')
print("Saved: feature_importance_rf_q2.png")
plt.close()
...
print("="*80)
print("\nGenerated files:")
print("  - analysis_data_q2.csv (merged dataset)")
print("  - model_comparison_q2.csv (performance metrics)")
print("  - eda_distributions_q2.png")
print("  - eda_correlation_matrix_q2.png")
print("  - eda_scatter_plots_q2.png")
print("  - eda_boxplots_q2.png")
print("  - model_predictions_comparison_q2.png")
print("  - model_performance_bars_q2.png")
print("  - feature_importance_rf_q2.png")
